...
export class AdminTrainingReminders {
  ...
  async getTrainingReminders(
    request: Request,
    response: Response
  ): Promise<void> {
    const userId = request.user.id as string;
    const trainerId = request.params.trainer_id
      ? request.params.trainer_id
      : userId;
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      const trainingReminders = await this.getTrainingRemindersFromDB(
        trainerId,
        client
      );
  ...
  async getTrainingRemindersFromDB(
    trainerId: string,
    client: pg.PoolClient
  ): Promise<Array<TrainingReminder>> {
    ...
    for (const row of rows) {
      const user: User = {
        ...
        registeredOn: moment
          .utc(row.registered_on)
          .format(constants.DATE_TIME_FORMAT)
      };
      user.timeZone = await usersTimeZones.getUserTimeZone(
        user.id as string,
        client
      );
      const lastReminderDateTime = moment
        .utc(row.last_reminder_date_time)
        .format(constants.DATE_TIME_FORMAT);
      const lastConversationDateTime = row.last_conversation_date_time
        ? moment
            .utc(row.last_conversation_date_time)
            .format(constants.DATE_TIME_FORMAT)
        : '';
      const certificateDate = moment
        .utc(row.registered_on)
        .tz(user.timeZone.name)
        .add(3, 'months')
        .format(constants.SHORT_DATE_FORMAT);
      const shouldShowTrainingReminder = this.getShouldShowTrainingReminder(
        lastReminderDateTime,
        lastConversationDateTime,
        user.timeZone,
        trainer.workdays
      );
      const isStepAdded = await this.getIsStepAdded(
        user.id as string,
        row.is_step_added,
        lastReminderDateTime,
        client
      );
      const allUserQuizzes = await usersQuizzes.getAllQuizzes(
        user.id as string,
        client
      );
      const hasPreviousRemainingQuizzes = this.getHasPreviousRemainingQuizzes(
        user,
        allUserQuizzes,
        quizSettings
      );
      const quizzes = await usersQuizzes.getQuizzesFromDB(
        user.id as string,
        client
      );
      const remainingQuizzes = helpers.getRemainingQuizzes(quizzes);
      const shouldShowRemainingQuizzes = this.getShouldShowRemainingQuizzes(
        hasPreviousRemainingQuizzes,
        remainingQuizzes,
        isStepAdded
      );
      if (
        shouldShowTrainingReminder &&
        (!isStepAdded || shouldShowRemainingQuizzes)
      ) {
        const firstReminderAtTimeZone = moment
          .utc(lastReminderDateTime)
          .tz(user.timeZone.name)
          .subtract(2, 'd')
          .format(constants.SHORT_DATE_FORMAT);
        const lastReminderAtTimeZone = moment
          .utc(lastReminderDateTime)
          .tz(user.timeZone.name)
          .format(constants.SHORT_DATE_FORMAT);
        const lastContactedDateTime = this.getLastContactedDateTime(
          row.last_contacted_date_time
        );
        ...
        trainingReminder.isOverdue = await this.getIsOverdue(
          user,
          allUserQuizzes,
          quizSettings,
          client
        ...
        const stepQuizzesText = this.getStepQuizzesText(
          user.isMentor as boolean,
          isStepAdded,
          remainingQuizzes,
          quizSettings
        );
        const weeklyQuizzesText = this.getWeeklyQuizzesText(
          user.isMentor as boolean,
          remainingQuizzes,
          quizSettings
        );
        const reminderToSend = this.getReminderToSend(
          trainingReminder,
          lastReminderDateTime,
          reminderText,
          trainer,
  ...
  getHasPreviousRemainingQuizzes(
    user: User,
    quizzes: Array<Quiz>,
    ...
    const registeredOn = moment
      .utc(user.registeredOn)
      .tz(timeZoneName)
      .startOf('day');
    const today = moment.utc().tz(timeZoneName).startOf('day');
    const timeSinceRegistration = today.subtract(1, 'd').diff(registeredOn);
    const weekNumber = Math.trunc(
      helpers.getDSTAdjustedDifferenceInDays(timeSinceRegistration) / 7
    );
    if (weekNumber == 0) {
      return false;
    } else {
      const weekStartDate = usersQuizzes.getWeekStartDate(
        registeredOn,
        weekNumber - 1,
        timeZone
      );
      const weekEndDate = usersQuizzes.getWeekEndDate(
        registeredOn,
        weekNumber - 1,
        timeZone
      );
      const quizzesBetweenDates = usersQuizzes.getQuizzesBetweenDates(
        quizzes,
        weekStartDate,
        weekEndDate,
        timeZone
      ...
      if (!isMentor) {
        isFirstQuizzesRoundCompleted = this.getIsFirstQuizzesRoundCompleted(
          quizzes,
          quizSettings
        );
      }
      const hasRemainingQuizzes = this.getHasRemainingQuizzes(
        user.isMentor as boolean,
        quizzesBetweenDates,
        quizSettings
      );
      return (
        ...
        (isMentor ||
          (!isMentor &&
            (weekNumber >= 11 ||
              (weekNumber < 11 && !isFirstQuizzesRoundCompleted))))
      );
    }
  }

  getShouldShowRemainingQuizzes(
    hasPreviousRemainingQuizzes: boolean,
    remainingQuizzes: number,
    isStepAdded: boolean
  ): boolean {
    return (
      (hasPreviousRemainingQuizzes && remainingQuizzes > 0) ||
      (!isStepAdded && remainingQuizzes > 0)
    );
  }
  ...
  async getIsOverdue(
    ...
    if (weekNumber < 2) {
      ...
      const isStepAdded = await this.getIsStepAdded(
        user.id as string,
        false,
        moment.utc(weekStartDate).format(constants.DATE_TIME_FORMAT),
        client
      );
      const quizzesBetweenDates = usersQuizzes.getQuizzesBetweenDates(
        quizzes,
      ...
      const hasRemainingQuizzes2WeeksBefore = this.getHasRemainingQuizzes(
        user.isMentor as boolean,
        quizzesBetweenDates,
        quizSettings
      );
      ...
      const currentQuizzes = await usersQuizzes.getQuizzesFromDB(
        user.id as string,
        client
      );
      const remainingQuizzes = helpers.getRemainingQuizzes(currentQuizzes);
      const areQuizzesSolved = !(
        hasRemainingQuizzes2WeeksBefore &&
        hasPreviousRemainingQuizzes &&
        remainingQuizzes > 0
      );
  ...
  getIsFirstQuizzesRoundCompleted(
    quizzes: Array<Quiz>,
    quizSettings: QuizSettings
  ): boolean {
  ...
  getStepQuizzesText(
    ...
    if (remainingQuizzes > 0) {
      ...
      if (remainingQuizzes == quizzesWeeklyCount) {
        ...
        const remainingQuizzesText =
          remainingQuizzes > 1 ? `${remainingQuizzes} quizzes` : `quiz`;
  ...
  getReminderToSend(
    ...
    trainer: User,
    user: User,
    weeklyQuizzesText: string,
    stepQuizzesText: string
  ): string {
    if (
      !reminderText ||
      (trainingReminder.lastConversationDateTime != '' &&
        moment
          .utc(trainingReminder.lastConversationDateTime)
          .format(constants.DATE_FORMAT) ==
          moment.utc().format(constants.DATE_FORMAT))
    ) {
      return '';
    }
    ...
    if (
      moment
        ...
        .tz(timeZoneName)
        .startOf('day')
        .isAfter(moment.utc().tz(timeZoneName).startOf('day'))
    ) {
      reminderText = reminderText
        .split('{certificate_date}')
        .join(
          certificateDate.tz(timeZoneName).format(constants.SHORT_DATE_FORMAT)
        );
    } else {
      reminderText = reminderText.split(' on {certificate_date}').join('');
    }
    reminderText = reminderText
      .split('{first_reminder_date}')
      .join(trainingReminder.firstReminderDate as string);
    reminderText = reminderText
      .split('{last_reminder_date}')
      .join(trainingReminder.lastReminderDate as string);
    reminderText = reminderText
      .split('{weekly_quizzes}')
      .join(weeklyQuizzesText);
    const stepQuizzesNotDone =
      stepQuizzesText
        .split('{add}')
        .join('added')
        .split('{solve}')
        .join('solved') + ' for the current week of training';
    reminderText = reminderText
      .split('{step_quizzes_not_done}')
      .join(stepQuizzesNotDone);
    const stepQuizzesToDo = stepQuizzesText
      .split('{add}')
      .join('add')
      .split('{solve}')
      .join('solve');
    reminderText = reminderText
      .split('{step_quizzes_to_do}')
      .join(stepQuizzesToDo);
    const stepQuizzesDoing = stepQuizzesText
      .split('{add}')
      .join('adding')
      .split('{solve}')
      .join('solving');
    reminderText = reminderText
      .split('{step_quizzes_doing}')
      .join(stepQuizzesDoing);
    let lastReminderDay = 'tomorrow';
    const lastReminderDaysToAdd = this.getLastReminderDaysToAdd(
      lastReminderDateTime,
      user.timeZone as TimeZone,
      trainer.workdays as number
    );
    ...
    reminderText = reminderText
      .split('{last_reminder_day}')
      .join(lastReminderDay);
    return reminderText;
  }

  getShouldShowTrainingReminder(
    lastReminderDateTime: string,
    lastConversationDateTime: string,
    userTimeZone: TimeZone,
    trainerWorkdays: number
  ): boolean {
    let shouldShowTrainingReminder = false;
    const now = moment.utc().startOf('day');
    const lastReminderStartOfDay = moment
      .utc(lastReminderDateTime)
      .startOf('day');
    let lastConversationStartOfDay;
    if (lastConversationDateTime != '') {
      lastConversationStartOfDay = moment
        .utc(lastConversationDateTime)
        .startOf('day');
    }
    const lastReminderDaysToAdd = this.getLastReminderDaysToAdd(
      lastReminderDateTime,
    ...
    if (
      now.diff(lastReminderStartOfDay, 'days') == 2 ||
      (now.diff(lastReminderStartOfDay, 'days') > 2 &&
        (!lastConversationStartOfDay ||
          lastConversationStartOfDay.diff(lastReminderStartOfDay, 'days') < 2 ||
          lastConversationStartOfDay.format(constants.DATE_FORMAT) ==
            now.format(constants.DATE_FORMAT))) ||
      moment.utc().tz(userTimeZone.name).format(constants.DATE_FORMAT) ==
        moment
          .utc(lastReminderDateTime)
          .tz(userTimeZone.name)
          .add(lastReminderDaysToAdd, 'd')
          .format(constants.DATE_FORMAT)
    ) {
  ...
  getLastReminderDaysToAdd(
    lastReminderDateTime: string,
    userTimeZone: TimeZone,
    trainerWorkdays: number
  ): number {
    let days = 6;
    const lastReminderDateTimeUser = moment
      .utc(lastReminderDateTime)
      .tz(userTimeZone.name);
  ...
  async getIsStepAdded(
    ...
    const { rows }: pg.QueryResult = await client.query(getIsStepAddedQuery, [
      userId,
      dateTime
    ]);
    if (rows.length > 0) {
  ...
  async getAllTrainingReminders(
    ...
    try {
      await client.query('BEGIN');
      const trainingReminders = await this.getAllTrainingRemindersFromDB(
        trainerId,
        client
      );
      response.status(200).json(trainingReminders);
  ...
  async getAllTrainingRemindersFromDB(
    trainerId: string,
    client: pg.PoolClient
  ): Promise<Array<TrainingReminder>> {
    const trainer = await users.getUserFromDB(trainerId, client);
    ...
    getTrainingRemindersQuery +=
      ' ORDER BY atr.last_contacted_date_time DESC NULLS LAST';
    const { rows }: pg.QueryResult = await client.query(
      getTrainingRemindersQuery,
      values
    );
  ...
  async addTrainingReminder(
    user: User,
    isStepAdded: boolean,
    remainingQuizzes: number,
    client: pg.PoolClient
  ): Promise<void> {
    const getTrainingRemindersQuery =
      'SELECT user_id, reminder_to_send FROM admin_training_reminders WHERE user_id = $1';
    const { rows }: pg.QueryResult = await client.query(
      getTrainingRemindersQuery,
      [user.id]
    );
    ...
    if (rows[0]) {
      if (!user.isMentor) {
        const reminderToSend = await this.getTrainingReminderSerialNumber(
          user,
          rows[0].reminder_to_send,
          false,
          client
        );
  ...
  async getTrainingReminderSerialNumber(
    user: User,
    previousSerialNumberString: string,
    isFromConversations: boolean,
    client: pg.PoolClient
  ): Promise<string> {
    const serialNumberFirstLetter = previousSerialNumberString[0];
    const previousSerialNumber = parseInt(
      previousSerialNumberString.substring(1, previousSerialNumberString.length)
    );
    if (
      !user.isMentor &&
    ...
    const serialNumberString =
      serialNumberFirstLetter + serialNumber.toString();
    const getTrainingRemindersTextsQuery =
      'SELECT id FROM admin_training_reminders_texts WHERE serial_number = $1';
    const { rows }: pg.QueryResult = await client.query(
      getTrainingRemindersTextsQuery,
      [serialNumberString]
    );
    if (rows[0]) {
  ...
  async addConversation(request: Request, response: Response): Promise<void> {
    ...
    try {
      ...
      const getConversationsQuery =
        'SELECT id, last_conversation_date_time FROM admin_conversations WHERE user_id = $1';
      const { rows }: pg.QueryResult = await client.query(
      ...
      response
        .status(200)
        .json(`Conversation has been added for user: ${userId}`);
      await client.query('COMMIT');
    } catch (error) {
      response.status(400).send(error);
      await client.query('ROLLBACK');
  ...
  async updateReminderToSend(
    ...
    const getTrainingRemindersQuery =
      'SELECT reminder_to_send FROM admin_training_reminders WHERE user_id = $1';
    const { rows }: pg.QueryResult = await client.query(
      getTrainingRemindersQuery,
      [userId]
    );
    ...
    if (
      ...
      (!lastConversationDateTime ||
        moment.utc().format(constants.DATE_FORMAT) !=
          moment.utc(lastConversationDateTime).format(constants.DATE_FORMAT))
    ) {
      const reminderToSend = await this.getTrainingReminderSerialNumber(
        user,
        rows[0].reminder_to_send,
    ...
    const lastContactedDateTime = moment
      .utc()
      .format(constants.DATE_TIME_FORMAT);
    await client.query(updateTrainingReminderQuery, [
  ...
  async updateLastContacted(
    ...
    try {
      ...
      response
        .status(200)
        .json(
          `Last contacted date/time has been updated for training reminder: ${id}`
        );
      await client.query('COMMIT');
    } catch (error) {
      response.status(400).send(error);
