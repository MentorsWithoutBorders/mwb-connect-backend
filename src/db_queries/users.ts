...
export class Users {
  ...
  async getUsers(request: Request, response: Response): Promise<void> {
    ...
    try {
      ...
      const getUsersQuery =
        'SELECT id, name, email, field_id, organization_id, is_mentor, is_available, available_from, registered_on FROM users ORDER BY id ASC';
  ...
  async getUser(request: Request, response: Response): Promise<void> {
    ...
    try {
      ...
      if (user.isMentor) {
        user.lessonsAvailability = await this.getUserLessonsAvailability(
  ...
  async getUserFromDB(id: string, client: pg.PoolClient): Promise<User> {
    ...
    const getUserQuery = `SELECT u.id AS user_id, u.name, u.email, u.phone_number, o.id AS organization_id, o.name AS organization_name, o.has_mentors, f.id AS field_id, f.name AS field_name, u.is_available, u.available_from, u.registered_on, ap.is_admin, ap.is_mentor, ap.is_center_manager, ap.is_org_manager
      FROM users u
      JOIN fields f
        ON u.field_id = f.id
  ...
  async getUserSubfields(
    ...
    for (const row of rows) {
      const skills: Array<Skill> = await this.getUserSkills(
        userId,
        row.id,
        client
      );
  ...
  async getUserLessonsAvailability(
    userId: string,
    client: pg.PoolClient
  ): Promise<LessonsAvailability> {
  ...
  async updateUser(request: Request, response: Response): Promise<void> {
    ...
    try {
      ...
      const updateUserQuery =
        'UPDATE users SET name = $1, email = $2, field_id = $3, is_available = $4, available_from = $5 WHERE id = $6';
      const values = [
        name,
        email,
        field?.id,
      ...
      await this.insertUserAvailabilities(
        userId,
        availabilities as Array<Availability>,
        client
      );
      if (isMentor) {
        await this.deleteUserSubfields(userId, client);
        await this.deleteUserSkills(userId, client);
        await this.insertUserSubfields(
          userId,
          field?.subfields as Array<Subfield>,
          client
        );
        await this.updateUserLessonsAvailability(
          userId,
          lessonsAvailability as LessonsAvailability,
          client
        );
      }
  ...
  async deleteUserAvailabilities(
    userId: string,
    client: pg.PoolClient
  ): Promise<void> {
  ...
  async insertUserSubfields(
    ...
    for (let i = 0; i < subfields.length; i++) {
      ...
      if (
        subfields[i].skills != null &&
        (subfields[i].skills as Array<Skill>).length > 0
      ) {
        await this.updateUserSkills(
          userId,
          subfields[i].id as string,
          subfields[i].skills as Array<Skill>,
          client
        );
      }
    }
  ...
  async insertUserAvailabilities(
    userId: string,
    ...
    for (const availability of availabilities) {
      ...
      if (
        moment(availability.time.to, 'h:ma').isBefore(
          moment(availability.time.from, 'h:ma')
        )
      ) {
      ...
      if (dayOfWeekConnected && timeToConnected != '00:00') {
        ...
        values = [
          userId,
          dayOfWeekConnected,
          timeFromConnected as string,
          timeToConnected as string,
          id
        ];
  ...
  async updateUserLessonsAvailability(
    userId: string,
    ...
    const values = [
      minIntervalInDays,
      lessonsAvailability.minIntervalUnit,
      lessonsAvailability.maxStudents,
      userId
    ];
  ...
  async deleteUser(request: Request, response: Response): Promise<void> {
    ...
    try {
      await client.query('BEGIN');
      const deleteTimeZoneQuery =
        'DELETE FROM users_timezones WHERE user_id = $1';
      await client.query(deleteTimeZoneQuery, [id]);
      const deleteAppFlagsQuery =
        'DELETE FROM users_app_flags WHERE user_id = $1';
      await client.query(deleteAppFlagsQuery, [id]);
      const deleteSupportRequestQuery =
        'DELETE FROM users_support_requests WHERE user_id = $1';
      await client.query(deleteSupportRequestQuery, [id]);
      const deleteSubfieldQuery =
        'DELETE FROM users_subfields WHERE user_id = $1';
      ...
      const deleteNotificationsSettingsQuery =
        'DELETE FROM users_notifications_settings WHERE user_id = $1';
      await client.query(deleteNotificationsSettingsQuery, [id]);
      const deleteLessonsStudentQuery =
        'DELETE FROM users_lessons_students WHERE student_id = $1';
      await client.query(deleteLessonsStudentQuery, [id]);
      const deleteLessonsNotesQuery =
        'DELETE FROM users_lessons_notes WHERE student_id = $1';
      await client.query(deleteLessonsNotesQuery, [id]);
      const deleteLessonsCanceledQuery =
        'DELETE FROM users_lessons_canceled WHERE user_id = $1';
      await client.query(deleteLessonsCanceledQuery, [id]);
      const deleteLessonsAvailabilitiesQuery =
        'DELETE FROM users_lessons_availabilities WHERE user_id = $1';
      await client.query(deleteLessonsAvailabilitiesQuery, [id]);
      if (user.isMentor) {
        const deleteLessonsQuery =
          'DELETE FROM users_lessons WHERE mentor_id = $1';
        await client.query(deleteLessonsQuery, [id]);
        const deleteCoursesMentorsQuery =
          'DELETE FROM users_courses_mentors WHERE mentor_id = $1';
        await client.query(deleteCoursesMentorsQuery, [id]);
        const deleteMentorsWaitingRequestsQuery =
          'DELETE FROM mentors_waiting_requests WHERE mentor_id = $1';
        await client.query(deleteMentorsWaitingRequestsQuery, [id]);
        const deleteMentorsPartnershipRequestsQuery =
          'DELETE FROM mentors_partnership_requests WHERE mentor_id = $1 OR partner_mentor_id = $1';
        await client.query(deleteMentorsPartnershipRequestsQuery, [id]);
        const deleteCoursesLessonsQuery =
          'DELETE FROM users_courses_lessons WHERE mentor_id = $1';
        await client.query(deleteCoursesLessonsQuery, [id]);
      } else {
        const deleteCoursesStudentsQuery =
          'DELETE FROM users_courses_students WHERE student_id = $1';
        await client.query(deleteCoursesStudentsQuery, [id]);
      }
      const deleteCoursesLessonsCanceledQuery =
        'DELETE FROM users_courses_lessons_canceled WHERE user_id = $1';
      await client.query(deleteCoursesLessonsCanceledQuery, [id]);
      const deleteLessonRequestsQuery =
        'DELETE FROM users_lesson_requests WHERE mentor_id = $1 OR student_id = $1';
      ...
      const deleteFCMTokenQuery =
        'DELETE FROM users_fcm_tokens WHERE user_id = $1';
      await client.query(deleteFCMTokenQuery, [id]);
      const deleteCertificatePauseQuery =
        'DELETE FROM users_certificates_pauses WHERE user_id = $1';
      await client.query(deleteCertificatePauseQuery, [id]);
      const deleteAvailabilitiesQuery =
        'DELETE FROM users_availabilities WHERE user_id = $1';
      await client.query(deleteAvailabilitiesQuery, [id]);
      const deleteInAppMessagesQuery =
        'DELETE FROM users_in_app_messages WHERE user_id = $1';
      await client.query(deleteInAppMessagesQuery, [id]);
      const deleteAppVersionQuery =
        'DELETE FROM users_app_versions WHERE user_id = $1';
      ...
      const deleteAdminAssignedUsersQuery =
        'DELETE FROM admin_assigned_users WHERE trainer_id = $1 OR assigned_user_id = $1';
      await client.query(deleteAdminAssignedUsersQuery, [id]);
      const deleteAdminAvailableUsersQuery =
        'DELETE FROM admin_available_users WHERE user_id = $1';
      await client.query(deleteAdminAvailableUsersQuery, [id]);
      const deleteAdminConversationsQuery =
        'DELETE FROM admin_conversations WHERE user_id = $1';
      await client.query(deleteAdminConversationsQuery, [id]);
      const deleteAdminPermissionsQuery =
        'DELETE FROM admin_permissions WHERE user_id = $1';
      await client.query(deleteAdminPermissionsQuery, [id]);
      const deleteAdminStudentsCertificatesQuery =
        'DELETE FROM admin_students_certificates WHERE user_id = $1';
      await client.query(deleteAdminStudentsCertificatesQuery, [id]);
      const deleteAdminTrainingRemindersQuery =
        'DELETE FROM admin_training_reminders WHERE user_id = $1';
  ...
  async deleteMentorLessonsDependencies(
    mentorId: string,
    client: pg.PoolClient
  ): Promise<void> {
    ...
    for (const row of rows) {
      const deleteLessonStudentsQuery =
        'DELETE FROM users_lessons_students WHERE lesson_id = $1';
      await client.query(deleteLessonStudentsQuery, [row.id]);
      const deleteLessonCanceledQuery =
        'DELETE FROM users_lessons_canceled WHERE lesson_id = $1';
