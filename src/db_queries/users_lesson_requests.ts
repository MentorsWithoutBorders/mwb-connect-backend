...
export class UsersLessonRequests {
  ...
  async addLessonRequest(request: Request, response: Response): Promise<void> {
    ...
    try {
      ...
      const lessonRequest = await this.addLessonRequestFromDB(
        studentId,
        client
      );
  ...
  async sendCustomLessonRequest(
    ...
    try {
      ...
      await this.insertStudentSubfield(
        studentId,
        subfield?.id as string,
        client
      );
      let availability;
      if (availabilities) {
        availability = availabilities[0];
        const timeTo = moment(availability.time.from, 'h:ma')
          .add(2, 'h')
          .format('h:ma');
      ...
      const availableMentors =
        await usersAvailableMentors.getAvailableMentorsFromDB(
          undefined,
          undefined,
          client
        );
      const availableLessonsMentors =
        await usersAvailableMentors.getAvailableLessonsMentorsFromDB(
          undefined,
          undefined,
          client
        );
      ...
      const previousLesson = await usersLessons.getPreviousLessonFromDB(
        studentId,
        client
      );
      ...
      if (
        this.getIsInAvailableLessonsMentors(mentorId, availableLessonsMentors)
      ) {
        const availableLesson = await usersLessons.getNextLessonFromDB(
          mentor.id as string,
          true,
          client
        );
        availableLesson.mentor = mentor;
        await this.addStudentToAvailableLesson(
          student,
          availableLesson,
          client
        );
        ...
        this.getIsInAvailableMentors(mentorId, availableMentors) ||
        isPreviousMentor
      ) {
        let lessonDateTime = moment.utc().add(2, 'd');
        while (
          lessonDateTime.format(constants.DAY_OF_WEEK_FORMAT) !=
  ...
  async updateStudentGoal(
    ...
    let { rows }: pg.QueryResult = await client.query(getUserFieldQuery, [
      studentId
    ]);
    let previousFieldId = '';
    if (rows[0]) {
  ...
  async cancelPreviousLessonRequests(
    studentId: string,
    client: pg.PoolClient
  ): Promise<void> {
    const updateLessonRequestsQuery =
      'UPDATE users_lesson_requests SET is_canceled = true WHERE student_id = $1';
    await client.query(updateLessonRequestsQuery, [studentId]);
  }

  getIsInAvailableMentors(
    mentorId: string,
    availableMentors: Array<User>
  ): boolean {
    let found = false;
  ...
  getIsInAvailableLessonsMentors(
    mentorId: string,
    availableLessonsMentors: Array<User>
  ): boolean {
    let found = false;
  ...
  async addLesson(lesson: Lesson, client: pg.PoolClient): Promise<Lesson> {
    ...
    const endRecurrence =
      isLessonRecurrent && lesson.endRecurrenceDateTime != undefined
        ? moment.utc(lesson.endRecurrenceDateTime)
        : null;
    const values = [
      lesson.mentor?.id,
      lesson.subfield?.id,
    ...
    await this.addStudent(
      addedLesson.id as string,
      student.id as string,
      client
    );
    return usersLessons.getNextLessonFromDB(
  ...
  async addStudent(
    ...
    const nextLesson = await usersLessons.getNextLessonFromDB(
      studentId,
      false,
      client
    );
  ...
  async addStudentSubfield(
    studentId: string,
    subfieldId: string,
    client: pg.PoolClient
  ): Promise<void> {
    const getSubfieldQuery =
      'SELECT user_id FROM users_subfields WHERE user_id = $1';
    const { rows }: pg.QueryResult = await client.query(getSubfieldQuery, [
      studentId
    ]);
    if (!rows[0]) {
      const insertSubfieldQuery = `INSERT INTO users_subfields (user_id, subfield_id)
  ...
  async deleteLessonRequest(
    lessonRequestId: string,
    client: pg.PoolClient
  ): Promise<void> {
    const deleteLessonRequestQuery =
      'DELETE FROM users_lesson_requests WHERE id = $1';
    await client.query(deleteLessonRequestQuery, [lessonRequestId]);
  }

  async rejectLessonRequest(
    request: Request,
    response: Response
  ): Promise<void> {
    const mentorId = request.user.id as string;
    ...
    try {
      await client.query('BEGIN');
      const getLessonRequestQuery =
        'SELECT student_id, is_canceled FROM users_lesson_requests WHERE id = $1';
      const { rows }: pg.QueryResult = await client.query(
        getLessonRequestQuery,
        [lessonRequestId]
      );
      if (rows && rows[0] && !rows[0].is_canceled) {
        ...
        const updateLessonRequestQuery =
          'UPDATE users_lesson_requests SET is_rejected = true WHERE mentor_id = $1 AND id = $2';
        await client.query(updateLessonRequestQuery, [
          mentorId,
          lessonRequestId
        ]);
        ...
        await usersWhatsAppMessages.sendWMLessonRequestRejected(
          lessonRequest,
          text
        );
      }
      response
        .status(200)
        .send(`Lesson request modified with ID: ${lessonRequestId}`);
      await client.query('COMMIT');
    } catch (error) {
      response.status(400).send(error);
      await client.query('ROLLBACK');
  ...
  async cancelLessons(
    ...
    let { rows }: pg.QueryResult = await client.query(getStudentLessonsQuery, [
      studentId
    ]);
    ...
    const commonLessonsIds = studentLessonsIds.filter((studentLessonId) =>
      mentorLessonsIds.includes(studentLessonId)
    );
    let lessonsIds = '';
  ...
  async cancelLessonRequest(
    ...
    try {
      const updateLessonRequestQuery =
        'UPDATE users_lesson_requests SET is_canceled = true WHERE student_id = $1 AND id = $2';
    ...
    try {
      const updateLessonRequestQuery = `UPDATE users_lesson_requests SET was_canceled_shown = $1, was_expired_shown = $2
        WHERE (mentor_id = $3 OR student_id = $3)
          AND id = $4`;
      await pool.query(updateLessonRequestQuery, [
        wasCanceledShown,
        wasExpiredShown,
        userId,
        lessonRequestId
  ...
  addAvailableLesson(
    lessonStudentsNumber: number,
    maxStudents: number,
    availableLesson: Lesson,
    availableLessons: Array<Lesson>
  ): Array<Lesson> {
  ...
  async addStudentToAvailableLesson(
    student: User,
    availableLesson: Lesson,
    client: pg.PoolClient
  ): Promise<void> {
    ...
    await this.addStudentSubfield(
      student.id as string,
      availableLesson.subfield?.id as string,
      client
    );
    await this.addStudentToLessonsCanceled(
      student.id as string,
      lessonId,
      availableLesson.mentor?.id as string,
      client
    );
    const mentor = await users.getUserFromDB(
      availableLesson.mentor?.id as string,
      client
    );
    availableLesson.mentor = mentor;
  ...
  async addStudentToLessonsCanceled(
    studentId: string,
    lessonId: string,
    mentorId: string,
    client: pg.PoolClient
