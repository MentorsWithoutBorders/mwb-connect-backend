...
export class AdminStudentsCertificates {
  ...
  async getStudentsCertificates(
    ...
    try {
      ...
      for (const row of rows) {
        ...
        const isTrainingCompleted = await this.getIsTrainingCompleted(
          student,
          client
        );
  ...
  async getIsTrainingCompleted(
    student: User,
    client: pg.PoolClient
  ): Promise<boolean> {
    const isStepAdded = !(await usersBackgroundProcesses.getShowStepReminder(
      student,
      client
    ));
    if (!isStepAdded) {
    ...
    const getQuizzesQuery =
      'SELECT number, is_correct FROM users_quizzes WHERE user_id = $1';
    const { rows }: pg.QueryResult = await client.query(getQuizzesQuery, [
      student.id || ''
    ]);
  ...
  async updateCertificateSent(
    ...
    const client = await pool.connect();
    try {
      const getCertificateSentQuery =
        'SELECT id FROM admin_students_certificates WHERE user_id = $1';
      const { rows }: pg.QueryResult = await client.query(
        getCertificateSentQuery,
        [studentId]
      );
      if (rows[0]) {
        const updateCertificateSentQuery = `UPDATE admin_students_certificates
          SET is_certificate_sent = $1 WHERE user_id = $2`;
        const values: any = [isCertificateSent, studentId];
        ...
        const insertCertificateSentQuery = `INSERT INTO admin_students_certificates (user_id, is_certificate_sent)
          VALUES ($1, $2)`;
        const values: any = [studentId, isCertificateSent];
      ...
      response
        .status(200)
        .json(`Certificate sent has been updated for user: ${studentId}`);
      await client.query('COMMIT');
    } catch (error) {
      response.status(400).send(error);
      await client.query('ROLLBACK');
  ...
  async getCertificateSent(
    ...
    try {
      const getCertificateSentQuery =
        'SELECT is_certificate_sent FROM admin_students_certificates WHERE user_id = $1';
