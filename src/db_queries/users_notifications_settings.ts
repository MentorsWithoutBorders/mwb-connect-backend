...
export class UsersNotificationsSettings {
  ...
  async getNotificationsSettings(
    ...
    try {
      const getNotificationsSettingsQuery =
        'SELECT enabled, time, training_reminders_enabled, training_reminders_time, start_course_reminders_enabled, start_course_reminders_date FROM users_notifications_settings WHERE user_id = $1';
      ...
      const trainingRemindersTime = trainingRemindersTimeDB.substring(
        0,
        trainingRemindersTimeDB.lastIndexOf(':')
      );
      const notificationsSettings: NotificationsSettings = {
        ...
        startCourseRemindersDate: rows[0].start_course_reminders_date
          ? moment
              .utc(rows[0].start_course_reminders_date)
  ...
  async updateNotificationsSettings(
    ...
    const {
      ...
      trainingRemindersTime,
      startCourseRemindersEnabled,
      startCourseRemindersDate
    }: NotificationsSettings = request.body;
    ...
    try {
      ...
      if (user.isMentor) {
        values = [
          trainingRemindersEnabledForUpdate,
          trainingRemindersTimeForUpdate,
          trainingRemindersEnabledForUpdate,
          trainingRemindersTimeForUpdate,
          startCourseRemindersEnabled,
          moment
            .utc(startCourseRemindersDate)
      ...
      usersSendEmails.sendEmailNotificationsSettingsUpdate(
        userId,
        trainingRemindersEnabledForUpdate
      );
      response
        .status(200)
        .send(`Notifications settings have been updated for user: ${userId}`);
