...
export class UsersQuizzes {
  ...
  async getQuizzesFromDB(
    ...
    const quizzesBetweenDates = this.getQuizzesBetweenDates(
      quizzes,
      weekStartDate,
      weekEndDate,
      userTimeZone
    ...
    if (user.isMentor) {
      ...
      if (weekNumber < constants.MENTOR_MAX_QUIZZES_SETS - 1) {
        const quizStartNumber = this.getQuizzesRemainingStartNumber(
          quizzes,
          registeredOn
        );
        quizzes = this.getAssignedQuizzesMentor(
          quizzes,
          quizzesBetweenDates,
          quizStartNumber,
          weeklyCount,
      ...
      if (weekNumber <= constants.STUDENT_MAX_QUIZZES_SETS * weeklyCount) {
        const quizzesSetNumber = this.getQuizzesSetNumber(
          quizzes,
          registeredOn,
          userTimeZone,
          weeklyCount
        );
        const quizStartNumber = this.getQuizStartNumber(
          quizzesSetNumber,
          weeklyCount
        );
        if (
          quizStartNumber <
          constants.STUDENT_MAX_QUIZZES_SETS * weeklyCount
        ) {
          quizzes = this.getAssignedQuizzesStudent(
            quizzesBetweenDates,
            quizStartNumber,
            weeklyCount,
            false
        ...
        const quizzesStartDate = moment
          .utc(registeredOn)
          .add((constants.STUDENT_MAX_QUIZZES_SETS * 2 + 1) * 7 + 1, 'days');
        const quizStartNumber = this.getQuizzesRemainingStartNumber(
          quizzes,
          quizzesStartDate
        );
  ...
  getAssignedQuizzesMentor(
    quizzes: Array<Quiz>,
    quizzesBetweenDates: Array<Quiz>,
    quizStartNumber: number,
    weeklyCount: number,
    isAllRemainingQuizzes: boolean
  ): Array<Quiz> {
    ...
    const quizEndNumber = isAllRemainingQuizzes
      ? weeklyCount * constants.WEEKS_PER_MONTH
      : quizStartNumber + weeklyCount - 1;
    ...
    for (let i = quizStartNumber; i <= quizEndNumber; i++) {
      if (
        !correctQuizzes.some((q) => q.number === i) ||
        correctQuizzesBetweenDates.some((q) => q.number === i)
      ) {
        const quiz: Quiz = {
          number: i
        };
  ...
  async getQuizNumberFromDB(
    ...
    if (!user.isMentor) {
      ...
      quizzes = this.getQuizzesBetweenDates(
        quizzes,
        weekStartDate,
        weekEndDate,
        userTimeZone
      );
      quizNumber = this.calculateQuizNumber(
        quizzes,
        weeklyCount,
        quizStartNumber,
        quizEndNumber
  ...
  getQuizzesRemainingStartNumber(
    quizzes: Array<Quiz>,
    quizzesStartDate: moment.Moment
  ): number {
    let startNumber = 1;
    for (const quiz of quizzes) {
      if (
        moment.utc(quiz.dateTime).isAfter(moment.utc(quizzesStartDate)) &&
        quiz.isCorrect &&
        quiz.number >= startNumber
      ) {
  ...
  getWeekStartDate(
    ...
    return moment
      .utc(registeredOn)
      .tz(timeZone.name)
      .add(weekNumber * 7 + extraDay, 'days');
  }
    ...
    for (const quiz of quizzes) {
      if (
        moment
          ...
          .tz(userTimeZone.name)
          .isAfter(
            moment.utc(weekStartDate).tz(userTimeZone.name).startOf('day')
          ) &&
        moment
          .utc(quiz.dateTime)
          .tz(userTimeZone.name)
          .isBefore(moment.utc(weekEndDate).tz(userTimeZone.name).endOf('day'))
      ) {
  ...
  calculateQuizNumber(
    ...
    weeklyCount: number,
    quizStartNumber: number,
    quizEndNumber: number
  ): number {
    let quizNumber = 0;
    const weekQuizzesSolved = this.getWeekQuizzesSolved(
      quizzes,
      quizStartNumber,
      quizEndNumber
    );
    if (
      quizStartNumber < constants.WEEKS_PER_MONTH * weeklyCount &&
  ...
  getQuizzesSetNumber(
    ...
    if (weekNumber < quizzesSetsEnd + 1) {
      ...
      quizzesSetsEnd =
        constants.STUDENT_MAX_QUIZZES_SETS * 2 +
        constants.STUDENT_MAX_QUIZZES_SETS +
        1;
  ...
  async addQuiz(request: Request, response: Response): Promise<void> {
    ...
    try {
      ...
      const quizzes = await this.getQuizzesFromDB(userId, client);
      const remainingQuizzes = helpers.getRemainingQuizzes(quizzes);
      await this.updateTrainingReminderRemainingQuizzes(
        userId,
        remainingQuizzes,
        client
      );
  ...
  async updateTrainingReminderRemainingQuizzes(
    userId: string,
