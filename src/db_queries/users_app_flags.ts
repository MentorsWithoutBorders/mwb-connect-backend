...
export class UsersAppFlags {
  ...
  async getAppFlags(request: Request, response: Response): Promise<void> {
    ...
    try {
      const getAppFlagsQuery =
        'SELECT is_training_enabled, is_mentoring_enabled FROM users_app_flags WHERE user_id = $1';
      const { rows }: pg.QueryResult = await pool.query(getAppFlagsQuery, [
        userId
      ]);
  ...
  async addAppFlags(request: Request, response: Response): Promise<void> {
    ...
    try {
      ...
      await this.addAppFlagsFromDB(
        userId,
        isTrainingEnabled as boolean,
        isMentoringEnabled as boolean,
        client
      );
      response
        .status(200)
        .send(`App flags have been inserted for user: ${userId}`);
