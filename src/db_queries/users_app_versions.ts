...
export class UsersAppVersions {
  ...
  async getAppVersion(
    ...
    const getAppVersionQuery =
      'SELECT major, minor, revision, build FROM users_app_versions WHERE user_id = $1';
    const { rows }: pg.QueryResult = await client.query(getAppVersionQuery, [
      userId
    ]);
  ...
  async addAppVersion(request: Request, response: Response): Promise<void> {
    ...
    try {
      ...
      response
        .status(200)
        .send(`App version has been updated for user: ${userId}`);
