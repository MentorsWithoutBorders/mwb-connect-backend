...
import LessonRecurrenceResult from '../models/lesson_recurrence_result_model';
...
export class UsersLessons {
  ...
  async getShouldStop(userId: string, client: pg.PoolClient): Promise<boolean> {
    const getShouldStopQuery =
      'SELECT id FROM users_lessons_stopped WHERE student_id = $1';
    const { rows }: pg.QueryResult = await client.query(getShouldStopQuery, [
      userId
    ]);
    return rows[0] ? true : false;
  }

  async getNextLessonFromDB(
    userId: string,
    isMentor: boolean,
    ...
    const { rows }: pg.QueryResult = await client.query(getNextLessonQuery, [
      userId
    ]);
    ...
    if (lessonRow) {
      const lesson: Lesson = {
        id: lessonRow.id,
        dateTime: moment
          .utc(lessonRow.date_time)
          .format(constants.DATE_TIME_FORMAT)
      };
      if (lessonRow.end_recurrence_date_time != null) {
        lesson.endRecurrenceDateTime = moment
          .utc(lessonRow.end_recurrence_date_time)
          .format(constants.DATE_TIME_FORMAT);
      }
      lessonRow.days_since_start = moment
        .utc()
        .diff(moment.utc(lessonRow.date_time), 'days');
      const lessonDateTime = await this.getNextLessonDateTime(
        lesson,
        userId,
        client
      );
  ...
  async getNextLessonDateTime(
    ...
    if (isLessonRecurrent) {
      return this.getNextLessonRecurrentDateTime(
        lesson,
        userId,
      ...
      return this.getNextLessonSingleDateTime(
        lesson,
        userId,
        lessonDateTime,
        client
  ...
  async getNextLessonRecurrentDateTime(
    lesson: Lesson,
    userId: string,
    endRecurrenceDateTime: moment.Moment,
    lessonDateTime: moment.Moment,
    client: pg.PoolClient
  ): Promise<string | undefined> {
    let now = moment.utc();
    ...
    if (endRecurrenceDateTime.isBefore(now)) {
      ...
      const nextValidLessonDateTime = await this.getNextValidLessonDateTime(
        lesson,
        userId,
        lessonDateTime,
        client
      ...
      if (
        nextValidLessonDateTime.isAfter(endRecurrence) ||
        nextValidLessonDateTime.isBefore(now)
      ) {
        return undefined;
      } else {
        return moment
          .utc(nextValidLessonDateTime)
          .format(constants.DATE_TIME_FORMAT);
      }
    }
  }

  async getNextLessonSingleDateTime(
    lesson: Lesson,
    userId: string,
    lessonDateTime: moment.Moment,
    client: pg.PoolClient
    ...
    if (lessonDateTime.isBefore(now)) {
      ...
      if (
        moment
          .utc(nextValidLessonDateTime)
          .format(constants.DATE_TIME_FORMAT) !=
        moment.utc(lessonDateTime).format(constants.DATE_TIME_FORMAT)
      ) {
        return undefined;
      } else {
  ...
  async getNextValidLessonDateTime(
    lesson: Lesson,
    userId: string,
    lessonDateTime: moment.Moment,
    client: pg.PoolClient
    ...
    const lessonCanceledDateTimes = await this.getLessonCanceledDateTimes(
      userId,
      lesson.id as string,
      client
    );
  ...
  async getLessonCanceledDateTimes(
    userId: string,
    lessonId: string,
    client: pg.PoolClient
    ...
    const { rows }: pg.QueryResult = await client.query(
      getLessonCanceledDateTimesQuery,
      values
    );
    let lessonCanceledDateTimes: Array<string> = [];
    rows.forEach(function (row) {
      lessonCanceledDateTimes.push(
        moment.utc(row.lesson_date_time).format(constants.DATE_TIME_FORMAT)
      );
    });
  ...
  async getPreviousLessonFromDB(
    ...
    if (lessonRow != null) {
      ...
      const lessonDateTime = await this.getPreviousLessonDateTime(
        lesson,
        userId,
        client
      );
  ...
  async getPreviousLessonDateTime(
    lesson: Lesson,
    userId: string,
    client: pg.PoolClient
  ): Promise<string | undefined> {
    ...
    if (isLessonRecurrent) {
      ...
      const previousValidLessonDateTime =
        await this.getPreviousValidLessonDateTime(
          lesson,
          userId,
          lessonDateTime,
          client
      ...
      if (endRecurrenceDateTime.isAfter(now)) {
        if (previousValidLessonDateTime.isBefore(now)) {
          return moment
            .utc(previousValidLessonDateTime)
            .format(constants.DATE_TIME_FORMAT);
        } else {
          return undefined;
        }
  ...
  async getPreviousValidLessonDateTime(
    lesson: Lesson,
    userId: string,
    lessonDateTime: moment.Moment,
    client: pg.PoolClient
  ...
  async cancelLesson(request: Request, response: Response): Promise<void> {
    ...
    try {
      ...
      if (isMentor) {
        ...
        lesson.mentor = await users.getUserFromDB(
      ...
      usersPushNotifications.sendPNLessonCanceled(
        lesson,
        student,
        isCancelAll,
        lessonsCanceled
      );
      usersSendEmails.sendEmailLessonCanceled(
        lesson,
        student,
        isCancelAll,
        lessonsCanceled
  ...
  isNextSingleLessonCanceled(
    nextLesson: Lesson,
    isLessonRecurrent: boolean
  ): boolean {
    return Object.keys(nextLesson).length == 0 && !isLessonRecurrent;
  }

  async cancelNextLessonNoStudents(
    lesson: Lesson,
    client: pg.PoolClient
  ): Promise<number> {
    let nextLessonMentor = await this.getNextLessonFromDB(
      lesson.mentor?.id as string,
      true,
      client
    );
    let lessonsCanceled = 0;
    while (
      Object.keys(nextLessonMentor).length > 0 &&
      nextLessonMentor.students?.length == 0 &&
      lessonsCanceled < 10
    ) {
      if (nextLessonMentor.students?.length == 0) {
        await this.cancelLessonFromDB(
          lesson.mentor?.id as string,
          nextLessonMentor,
          client
        );
        nextLessonMentor = await this.getNextLessonFromDB(
          lesson.mentor?.id as string,
          true,
          client
        );
  ...
  async cancelLessonFromDB(
    ...
    if (lesson.dateTime) {
      ...
      await this.cancelRecurrentLesson(
        userId,
        isMentor,
        lesson,
        canceledDateTime,
  ...
  async cancelSingleLesson(
    ...
    nextLessonMentor: Lesson,
    students: Array<User>,
    client: pg.PoolClient
  ): Promise<void> {
    const isCanceled = await this.isLessonAlreadyCanceled(
      userId,
      lesson.id as string,
      lesson.dateTime as string,
      client
  ...
  async cancelRecurrentLesson(
    ...
    if (isMentor) {
      const updateMentorLessonQuery =
        'UPDATE users_lessons SET is_canceled = true, canceled_date_time = $1 WHERE id = $2';
      await client.query(updateMentorLessonQuery, [
        canceledDateTime,
        lesson.id
      ]);
      ...
      const nextLessonStudent = await this.getNextLessonFromDB(
        userId,
        false,
        client
      );
      ...
      await client.query(updateStudentLessonQuery, [
        canceledDateTime,
        lesson.id,
        userId
      ]);
  ...
  async cancelUserLessons(
    ...
    while (lesson.id != null && lessonDateTime.isSameOrBefore(endDateTime)) {
      const isCanceled = await this.isLessonAlreadyCanceled(
        userId,
        lesson.id,
        lessonDateTime.format(constants.DATE_TIME_FORMAT),
        client
      );
      if (!isCanceled) {
        const insertLessonCanceledQuery = `INSERT INTO users_lessons_canceled (user_id, lesson_id, lesson_date_time, canceled_date_time)
          VALUES ($1, $2, $3, $4)`;
        const canceledDateTime = moment
          .utc()
          .format(constants.DATE_TIME_FORMAT);
        const values = [userId, lesson.id, lessonDateTime, canceledDateTime];
        await client.query(insertLessonCanceledQuery, values);
  ...
  async setLessonUrl(request: Request, response: Response): Promise<void> {
    ...
    try {
      ...
      const updateLessonQuery =
        'UPDATE users_lessons SET meeting_url = $1 WHERE mentor_id = $2 AND id = $3';
  ...
  async setLessonRecurrence(
    ...
    try {
      ...
      for (const student of students) {
        ...
        const hasLessonRequestStudent = await this.getHasLessonRequestStudent(
          student.id as string,
          mentorId,
          client
        );
        if (
          (nextLessonStudent.id != undefined &&
            nextLessonStudent.id != lessonId) ||
          hasLessonRequestStudent
        ) {
          await this.cancelPreviousLesson(
            student.id as string,
            lesson.id as string,
            client
          );
      ...
      if (
        studentsRemaining.length > 0 &&
        nextLesson.endRecurrenceDateTime != endRecurrenceDateTime
      ) {
        const updateLessonQuery =
          'UPDATE users_lessons SET end_recurrence_date_time = $1 WHERE mentor_id = $2 AND id = $3';
        const endRecurrence =
          endRecurrenceDateTime != undefined
            ? moment.utc(endRecurrenceDateTime)
            : null;
        await client.query(updateLessonQuery, [
          endRecurrence,
          mentorId,
          lessonId
        ]);
        ...
        await usersWhatsAppMessages.sendWMLessonRecurrenceUpdated(
          studentsRemaining
        );
      }
      ...
      response.status(200).send(lessonRecurrenceResult);
      await client.query('COMMIT');
    } catch (error) {
      response.status(400).send(error);
      await client.query('ROLLBACK');
  ...
  async getHasLessonRequestStudent(
    studentId: string,
    mentorId: string,
    client: pg.PoolClient
  ): Promise<boolean> {
    ...
    const { rows }: pg.QueryResult = await client.query(getHasLessonRequest, [
      studentId,
      mentorId
    ]);
  ...
  async deleteMentorLessonRequests(
    mentorId: string,
    client: pg.PoolClient
  ): Promise<void> {
    const deleteMentorLessonRequestsQuery =
      'DELETE FROM users_lesson_requests WHERE is_previous_mentor IS true AND mentor_id = $1';
  ...
  async addStudentsSkills(request: Request, response: Response): Promise<void> {
    ...
    try {
      ...
      for (const student of students) {
        ...
        await usersSkills.addUserSkillsToDB(
          student.id as string,
          subfieldId,
          skills,
          client
  ...
  async getLessonSubfieldId(
    lessonId: string,
    client: pg.PoolClient
  ): Promise<string> {
    const getLessonQuery = `SELECT subfield_id FROM users_lessons WHERE id = $1`;
    const { rows }: pg.QueryResult = await client.query(getLessonQuery, [
      lessonId
    ]);
    return rows[0].subfield_id;
  }

  async getStudentLessonNotes(
    request: Request,
    response: Response
  ): Promise<void> {
    ...
    try {
      ...
      const { rows }: pg.QueryResult = await pool.query(getLessonNotesQuery, [
        studentId
      ]);
  ...
  async getLessonGuideRecommendations(
    request: Request,
    response: Response
  ): Promise<void> {
    const userId = request.user.id as string;
    ...
    try {
      ...
      guideRecommendations.push(
        await this.getGeneralGuideRecommendations(client)
      );
      const fieldGuideRecommendation = await this.getFieldGuideRecommendations(
        field as Field,
        client
      );
      ...
      const subfieldGuideRecommendation =
        await this.getSubfieldGuideRecommendations(lessonSubfield, client);
  ...
  async getGeneralGuideRecommendations(
    client: pg.PoolClient
  ...
  async getFieldGuideRecommendations(
    field: Field,
    client: pg.PoolClient
  ): Promise<GuideRecommendation> {
    const getGuideRecommendationsQuery = `SELECT text
  ...
  async getSubfieldGuideRecommendations(
    subfield: Subfield,
    client: pg.PoolClient
  ): Promise<GuideRecommendation> {
    const getGuideRecommendationsQuery = `SELECT text
  ...
  async getLessonGuideTutorials(
    ...
    try {
      ...
      for (const guideTutorial of guideTutorials) {
        guideTutorial.skills = this.replaceSkillsIdsWithNames(
          guideTutorial.skills,
          skills
        );
      }
  ...
  async getLessonSkills(
    ...
    const subfieldSkills = await skillsQueries.getSkillsFromDB(
      subfieldId,
      client
    );
  ...
  groupGuideTutorials(
    guideTutorialsInitial: Array<GuideTutorial>
  ): Array<GuideTutorial> {
  ...
  replaceSkillsIdsWithNames(
    skillsIds: Array<string>,
    skills: Array<Skill>
  ): Array<string> {
  ...
  async setLessonPresenceMentor(
    ...
    try {
      const updateLessonQuery =
        'UPDATE users_lessons SET is_mentor_present = $1 WHERE id = $2';
