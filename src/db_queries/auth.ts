...
export class Auth {
  ...
  async signUp(request: Request, response: Response): Promise<void> {
    ...
    try {
      ...
      const approvedUser: ApprovedUser = await approvedUsers.getApprovedUser(
        email,
        client
      );
      if (approvedUser.email == '') {
        response
          ...
          .send({
            message:
              'You have to be a student from one of our partner NGOs or an employee of one of our partner companies.'
          });
        return;
      }
      ...
      const values = [
        ...
        approvedUser.organization != null
          ? (approvedUser.organization.id as string)
          : '',
      ...
      await this.setDefaultUserProfile(
        userId,
        approvedUser.isMentor as boolean,
        client
      );
      await usersAppFlags.addAppFlagsFromDB(userId, true, true, client);

      const isStudent = !(
        approvedUser.isMentor ||
        approvedUser.isCenterManager ||
        approvedUser.isOrgManager
      );
      if (isStudent) {
        await usersGoals.addGoalToDB(
          userId,
          approvedUser.goal as string,
          client
        );
      }
  ...
  async setDefaultUserProfile(
    ...
    const getDefaultUserQuery =
      'SELECT lessons_availability_min_interval_in_days, lessons_availability_min_interval_unit, lessons_availability_max_students, training_reminders_enabled, training_reminders_time, start_course_reminders_enabled, is_available FROM user_default_profile';
    ...
    const notificationsSettings: NotificationsSettings = {
      ...
      startCourseRemindersDate: moment
    ...
    if (isMentor) {
      const insertUserLessonsAvailabilityQuery = `INSERT INTO users_lessons_availabilities (user_id, min_interval_in_days, min_interval_unit, max_students)
        VALUES ($1, $2, $3, $4)`;
      await client.query(insertUserLessonsAvailabilityQuery, [
        userId,
        lessonsAvailability.minInterval,
        lessonsAvailability.minIntervalUnit,
        lessonsAvailability.maxStudents
      ]);
    }
    const startCourseRemindersEnabled = isMentor
      ? notificationsSettings.startCourseRemindersEnabled
      : null;
    const startCourseRemindersDate = isMentor
      ? notificationsSettings.startCourseRemindersDate
      : null;
    const insertNotificationsSettingsQuery = `INSERT INTO users_notifications_settings (user_id, training_reminders_enabled, training_reminders_time, start_course_reminders_enabled, start_course_reminders_date)
      VALUES ($1, $2, $3, $4, $5)`;
    await client.query(insertNotificationsSettingsQuery, [
      userId,
      notificationsSettings.trainingRemindersEnabled,
      notificationsSettings.trainingRemindersTime,
      startCourseRemindersEnabled,
  ...
  async setRefreshToken(
    ...
    const getRefreshTokenQuery =
      'SELECT user_id FROM users_refresh_tokens WHERE user_id = $1';
    const { rows }: pg.QueryResult = await client.query(getRefreshTokenQuery, [
      userId
    ]);
    if (!rows[0]) {
  ...
  async addRefreshToken(
    ...
    const insertRefreshTokenQuery = `INSERT INTO 
      ...
      VALUES ($1, $2)`;
    const values: Array<string> = [userId, refreshToken];
  ...
  async updateRefreshToken(
    ...
    const values: Array<string> = [refreshToken, userId];
  ...
  async getAccessToken(request: Request, response: Response): Promise<void> {
    ...
    try {
      await client.query('BEGIN');
      const getRefreshTokenQuery =
        'SELECT user_id FROM users_refresh_tokens WHERE user_id = $1 AND refresh_token = $2';
